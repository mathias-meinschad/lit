...
void Commit::toFile(std::string fileName)
{
	std::ofstream out_file(fileName + ".commit");
	if (out_file.is_open()) {
		// todo: add logic to parse file here
		char buffer[80];
		out_file << revisionNumber << std::endl;
		out_file << parentCommit << std::endl;
		out_file << commitMsg << std::endl;
		// todo: parsing time from file is not working, but probably not needed
		...
		// todo: check why we use a reference type here
