...
Differences::Differences(std::string srcFile, std::string destFile, std::string diffOutput)
...
void Commit::toFile(const std::string &fileName)
{
	...
	if (out_file.is_open()) {
        ...
        out_file << parentCommit2 << std::endl;
...
Commit::Commit(const std::string &fileName)
{
	...
	if (in_file.is_open()) {
		...
		while (getline(in_file, line)) {
			...
			if (!diffOutput.empty()) {
				Differences litDifference(srcFile, destFile, diffOutput);
...
Commit::Commit(std::string revisionNumber, std::string parentCommit, std::string parentCommit2, std::string commitMsg,
               tm date, std::list<Differences> differences, std::list<std::string> addedFiles,
               std::list<std::string> removedFiles)
    : revisionNumber(revisionNumber), parentCommit(parentCommit), parentCommit2(parentCommit2), commitMsg(commitMsg),
      date(date), differences(differences), addedFiles(addedFiles), removedFiles(removedFiles)
{
}